...
# Ensure we can import blazer
sys.path.append(os.path.dirname(os.path.abspath(__file__)))

import blazer
from authenticator import FaceAuthenticator
from kasa_agent import KasaAgent
from point_e_agent import get_point_e_agent
...
async def start_audio(sid, data=None):
    ...
    def on_cad_status(status):
        # status can be: 
        # - a string like "generating" (from blazer.py handle_cad_request)
    ...
    try:
        ...
        print("Emitting 'Blazer Started'")
        await sio.emit('status', {'msg': 'Blazer Started'})
        ...
        print(f"CRITICAL ERROR STARTING BLAZER: {e}")
        import traceback
        traceback.print_exc()
...
async def user_input(sid, data):
    ...
    # If audio loop is not running, start it first
    if not audio_loop or not audio_loop.session:
        print("[SERVER DEBUG] Audio loop not running. Starting it for text input...")
        # Start audio session automatically
        await start_audio(sid, data=None)
        # Wait a moment for session to initialize
        await asyncio.sleep(1)
    
    if not audio_loop:
        print("[SERVER DEBUG] [Error] Failed to initialize audio loop.")
        await sio.emit('error', {'msg': 'Failed to start audio session'}, to=sid)
        return

    if not audio_loop.session:
        print("[SERVER DEBUG] [Error] Session failed to initialize.")
        await sio.emit('error', {'msg': 'Audio session not ready'}, to=sid)
        return
...
async def generate_cad(sid, data):
    # data: { prompt: "make a cube", save_path: "C:/path/to/file.ply" }
    prompt = data.get('prompt')
    save_path = data.get('save_path')  # Optional - from file explorer dialog
    print(f"Received generate_cad request: '{prompt}'")
    
    try:
        await sio.emit('status', {'msg': 'Generating 3D model with Point-E...'})
        await sio.emit('cad_status', {'status': 'generating'})
        
        # Get Point-E agent
        point_e = get_point_e_agent()
        
        if not point_e.available:
            await sio.emit('error', {'msg': "Point-E not available. Please install torch and Point-E."})
            return
        
        # Generate 3D model
        # If no save path provided, use project folder
        if not save_path and audio_loop:
            cad_output_dir = audio_loop.project_manager.get_current_project_path() / "cad"
            cad_output_dir.mkdir(parents=True, exist_ok=True)
            # Create safe filename from prompt
            safe_name = "".join(c if c.isalnum() or c in (' ', '-', '_') else '' for c in prompt)[:50]
            save_path = str(cad_output_dir / f"{safe_name}.ply")
        
        result = point_e.generate_3d(prompt, save_path=save_path)
        
        if result.get('success'):
            print(f"3D model generated: {result.get('file_path')}")
            ...
            await sio.emit('cad_data', {
                ...
                'format': result.get('format', 'ply')
            })
            
            await sio.emit('status', {'msg': f"3D model saved to {result.get('file_path')}"})
            await sio.emit('cad_status', {'status': 'complete'})
        else:
            await sio.emit('error', {'msg': f"Failed to generate: {result.get('error')}"})
            await sio.emit('cad_status', {'status': 'error'})
...
async def prompt_web_agent(sid, data):
    ...
    try:
        ...
        await audio_loop.web_agent.run_task(prompt)
