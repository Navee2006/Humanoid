...
# Load .env from parent directory (since backend runs with cwd=backend/)
...
class CadAgent:
    def __init__(self, on_thought=None, on_status=None):
        ...
        # Using Gemini 2.0 Flash for fast code generation with thinking support
        ...
        try:
            ...
            for attempt in range(max_retries):
                ...
                if not self.gemini_enabled or self.client is None:
                    raise RuntimeError("Gemini API key not configured; CadAgent cannot generate prototypes.")
                raw_content = ""
