...
GEMINI_DISABLE_LIVE = os.getenv("GEMINI_DISABLE_LIVE", "false").lower() in ("1", "true", "yes")

# Treat obvious placeholders as unset
_placeholder_values = {"", None, "DUMMY_KEY", "REPLACE_ME", "YOUR_KEY_HERE"}

if GEMINI_DISABLE_LIVE:
    print("[BLAZER DEBUG] GEMINI_DISABLE_LIVE is set — disabling Gemini Live connections.")
    client = None
    GEMINI_ENABLED = False
elif not GEMINI_API_KEY or GEMINI_API_KEY in _placeholder_values or GEMINI_API_KEY.strip().upper().startswith("DUMMY"):
    print("[BLAZER DEBUG] No valid GEMINI_API_KEY found — Gemini Live disabled.")
    client = None
    GEMINI_ENABLED = False
else:
    try:
        ...
        GEMINI_ENABLED = True
        print("[BLAZER DEBUG] Gemini client initialized (live enabled).")
    except Exception as e:
        print(f"[BLAZER DEBUG] Failed to initialize Gemini client: {e}. Gemini Live disabled.")
        client = None
        GEMINI_ENABLED = False
...
config = types.LiveConnectConfig(
    ...
    system_instruction="Your name is Blazer, which stands for Blazing Laser Artificial Intelligence. "
        "You have a witty and charming personality. "
        "Your creator is Arun Kumar, and you address him as 'Sir'. "
...
class AudioLoop:
    ...
    def update_permissions(self, new_perms):
        print(f"[BLAZER DEBUG] [CONFIG] Updating tool permissions: {new_perms}")
    ...
    def resolve_tool_confirmation(self, request_id, confirmed):
        print(f"[BLAZER DEBUG] [RESOLVE] resolve_tool_confirmation called. ID: {request_id}, Confirmed: {confirmed}")
        if request_id in self._pending_confirmations:
            future = self._pending_confirmations[request_id]
            if not future.done():
                print(f"[BLAZER DEBUG] [RESOLVE] Future found and pending. Setting result to: {confirmed}")
                future.set_result(confirmed)
            else:
                 print(f"[BLAZER DEBUG] [WARN] Request {request_id} future already done. Result: {future.result()}")
        else:
            print(f"[BLAZER DEBUG] [WARN] Confirmation Request {request_id} not found in pending dict. Keys: {list(self._pending_confirmations.keys())}")
    ...
    def clear_audio_queue(self):
        ...
        try:
            ...
            while not self.audio_in_queue.empty():
                ...
                count += 1
            if count > 0:
                print(f"[BLAZER DEBUG] [AUDIO] Cleared {count} chunks from playback queue due to interruption.")
        except Exception as e:
            print(f"[BLAZER DEBUG] [ERR] Failed to clear audio queue: {e}")
            ...
            try:
                ...
                if rms > VAD_THRESHOLD:
                    ...
                    if not self._is_speaking:
                        ...
                        print(f"[BLAZER DEBUG] [VAD] Speech Detected (RMS: {rms}). Sending Video Frame.")
                        
                        # Send ONE frame
                        if self._latest_image_payload and self.out_queue:
                            await self.out_queue.put(self._latest_image_payload)
                        else:
                            print(f"[BLAZER DEBUG] [VAD] No video frame available to send.")
                    ...
                    if self._is_speaking:
                        if self._silence_start_time is None:
                            ...
                            print(f"[BLAZER DEBUG] [VAD] Silence detected. Resetting speech state.")
                            self._is_speaking = False
                            self._silence_start_time = None
    ...
    async def handle_cad_request(self, prompt):
        print(f"[BLAZER DEBUG] [CAD] Background Task Started: handle_cad_request('{prompt}')")
        if self.on_cad_status:
        ...
        if self.project_manager.current_project == "temp":
            ...
            print(f"[BLAZER DEBUG] [CAD] Auto-creating project: {new_project_name}")
            ...
            if success:
                ...
                try:
                    ...
                    print(f"[BLAZER DEBUG] [ERR] Failed to notify auto-project: {e}")
        ...
        if cad_data:
            print(f"[BLAZER DEBUG] [OK] CadAgent returned data successfully.")
            print(f"[BLAZER DEBUG] [INFO] Data Check: {len(cad_data.get('vertices', []))} vertices, {len(cad_data.get('edges', []))} edges.")
            
            if self.on_cad_data:
                print(f"[BLAZER DEBUG] [SEND] Dispatching data to frontend callback...")
                self.on_cad_data(cad_data)
                print(f"[BLAZER DEBUG] [SENT] Dispatch complete.")
            ...
            try:
                ...
                print(f"[BLAZER DEBUG] [NOTE] Sent completion notification to model.")
            except Exception as e:
                 print(f"[BLAZER DEBUG] [ERR] Failed to send completion notification: {e}")

        else:
            print(f"[BLAZER DEBUG] [ERR] CadAgent returned None.")
    ...
    async def handle_write_file(self, path, content):
        print(f"[BLAZER DEBUG] [FS] Writing file: '{path}'")
        ...
        if self.project_manager.current_project == "temp":
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            new_project_name = f"Project_{timestamp}"
            print(f"[BLAZER DEBUG] [FS] Auto-creating project: {new_project_name}")
        ...
        print(f"[BLAZER DEBUG] [FS] Resolved path: '{final_path}'")
        ...
        print(f"[BLAZER DEBUG] [FS] Result: {result}")
        try:
             await self.session.send(input=f"System Notification: {result}", end_of_turn=True)
        except Exception as e:
             print(f"[BLAZER DEBUG] [ERR] Failed to send fs result: {e}")

    async def handle_read_directory(self, path):
        print(f"[BLAZER DEBUG] [FS] Reading directory: '{path}'")
        try:
    ...
    async def handle_read_file(self, path):
        print(f"[BLAZER DEBUG] [FS] Reading file: '{path}'")
        try:
            if not os.path.exists(path):
    ...
    async def handle_web_agent_request(self, prompt):
        print(f"[BLAZER DEBUG] [WEB] Web Agent Task: '{prompt}'")
        ...
        print(f"[BLAZER DEBUG] [WEB] Web Agent Task Returned: {result}")
        ...
        try:
             ...
             print(f"[BLAZER DEBUG] [ERR] Failed to send web agent result to model: {e}")
                            ...
                            if transcript:
                                ...
                                if transcript != self._last_output_transcription:
                                    ...
                                    if delta:
                                        # Send to frontend (Streaming)
                                        if self.on_transcription:
                                             self.on_transcription({"sender": "BLAZER", "text": delta})
                                        
                                        # Buffer for Logging
                                        if self.chat_buffer["sender"] != "BLAZER":
                                            # Flush previous
                                            if self.chat_buffer["sender"] and self.chat_buffer["text"].strip():
                                                self.project_manager.log_chat(self.chat_buffer["sender"], self.chat_buffer["text"])
                                            # Start new
                                            self.chat_buffer = {"sender": "BLAZER", "text": delta}
                                        else:
                                            # Append
                                            self.chat_buffer["text"] += delta
                        ...
                        for fc in response.tool_call.function_calls:
                            if fc.name in ["generate_cad", "run_web_agent", "write_file", "read_directory", "read_file", "create_project", "switch_project", "list_projects", "list_smart_devices", "control_light", "discover_printers", "print_stl", "get_print_status", "iterate_cad"]:
                                ...
                                if not confirmation_required:
                                    print(f"[BLAZER DEBUG] [TOOL] Permission check: '{fc.name}' -> AUTO-ALLOW")
                                    ...
                                    print(f"[BLAZER DEBUG] [STOP] Requesting confirmation for '{fc.name}' (ID: {request_id})")
                                    ...
                                    print(f"[BLAZER DEBUG] [CONFIRM] Request {request_id} resolved. Confirmed: {confirmed}")

                                    if not confirmed:
                                        print(f"[BLAZER DEBUG] [DENY] Tool call '{fc.name}' denied by user.")
                                ...
                                if fc.name == "generate_cad":
                                    print(f"\n[BLAZER DEBUG] --------------------------------------------------")
                                    print(f"[BLAZER DEBUG] [TOOL] Tool Call Detected: 'generate_cad'")
                                    print(f"[BLAZER DEBUG] [IN] Arguments: prompt='{prompt}'")
                                    ...
                                    print(f"[BLAZER DEBUG] [TOOL] Tool Call: 'run_web_agent' with prompt='{prompt}'")
                                    ...
                                    print(f"[BLAZER DEBUG] [RESPONSE] Sending function response: {function_response}")
                                    function_responses.append(function_response)
                                    ...
                                    print(f"[BLAZER DEBUG] [TOOL] Tool Call: 'write_file' path='{path}'")
                                    ...
                                    path = fc.args["path"]
                                    print(f"[BLAZER DEBUG] [TOOL] Tool Call: 'read_directory' path='{path}'")
                                    ...
                                    path = fc.args["path"]
                                    print(f"[BLAZER DEBUG] [TOOL] Tool Call: 'read_file' path='{path}'")
                                    ...
                                    name = fc.args["name"]
                                    print(f"[BLAZER DEBUG] [TOOL] Tool Call: 'create_project' name='{name}'")
                                    ...
                                    name = fc.args["name"]
                                    print(f"[BLAZER DEBUG] [TOOL] Tool Call: 'switch_project' name='{name}'")
                                    ...
                                    if success:
                                        ...
                                        print(f"[BLAZER DEBUG] [PROJECT] Sending project context to AI ({len(context)} chars)")
                                        try:
                                            await self.session.send(input=f"System Notification: {msg}\n\n{context}", end_of_turn=False)
                                        except Exception as e:
                                            print(f"[BLAZER DEBUG] [ERR] Failed to send project context: {e}")
                                    function_response = types.FunctionResponse(
                                        id=fc.id, name=fc.name, response={"result": msg}
                                    )
                                    function_responses.append(function_response)
                                    ...
                                    print(f"[BLAZER DEBUG] [TOOL] Tool Call: 'list_projects'")
                                    ...
                                    print(f"[BLAZER DEBUG] [TOOL] Tool Call: 'list_smart_devices'")
                                    ...
                                    print(f"[BLAZER DEBUG] [TOOL] Tool Call: 'control_light' Target='{target}' Action='{action}'")
                                    ...
                                    print(f"[BLAZER DEBUG] [TOOL] Tool Call: 'discover_printers'")
                                    ...
                                    print(f"[BLAZER DEBUG] [TOOL] Tool Call: 'print_stl' STL='{stl_path}' Printer='{printer}'")
                                    ...
                                    printer = fc.args["printer"]
                                    print(f"[BLAZER DEBUG] [TOOL] Tool Call: 'get_print_status' Printer='{printer}'")
                                    ...
                                    print(f"[BLAZER DEBUG] [TOOL] Tool Call: 'iterate_cad' Prompt='{prompt}'")
                                    ...
                                    if cad_data:
                                        print(f"[BLAZER DEBUG] [OK] CadAgent iteration returned data successfully.")
                                        
                                        # Dispatch to frontend
                                        if self.on_cad_data:
                                            print(f"[BLAZER DEBUG] [SEND] Dispatching iterated CAD data to frontend...")
                                            self.on_cad_data(cad_data)
                                            print(f"[BLAZER DEBUG] [SENT] Dispatch complete.")
                                        ...
                                        print(f"[BLAZER DEBUG] [ERR] CadAgent iteration returned None.")
    ...
    async def run(self, start_message=None):
        ...
        if not GEMINI_ENABLED or client is None:
            print("[BLAZER DEBUG] GEMINI_API_KEY not set or invalid — Gemini Live disabled. AudioLoop.run will exit.")
            return

        while not self.stop_event.is_set():
            try:
                print(f"[BLAZER DEBUG] [CONNECT] Connecting to Gemini Live API...")
                async with (
                    ...
                    if not is_reconnect:
                        if start_message:
                            print(f"[BLAZER DEBUG] [INFO] Sending start message: {start_message}")
                        ...
                        print(f"[BLAZER DEBUG] [RECONNECT] Connection restored.")
                        # Restore Context
                        print(f"[BLAZER DEBUG] [RECONNECT] Fetching recent chat history to restore context...")
                        ...
                        print(f"[BLAZER DEBUG] [RECONNECT] Sending restoration context to model...")
                ...
                print(f"[BLAZER DEBUG] [STOP] Main loop cancelled.")
                break
                
            except Exception as e:
                # This catches the ExceptionGroup from TaskGroup or direct exceptions
                print(f"[BLAZER DEBUG] [ERR] Connection Error: {e}")
                
                if self.stop_event.is_set():
                    break
                
                print(f"[BLAZER DEBUG] [RETRY] Reconnecting in {retry_delay} seconds...")
