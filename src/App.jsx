...
function App() {
    ...
    const [isLockScreenVisible, setIsLockScreenVisible] = useState(false); // Never show lock screen

    // Local state for tracking settings, also init from local storage
    const [faceAuthEnabled, setFaceAuthEnabled] = useState(false); // Face auth disabled
    ...
    useEffect(() => {
        ...
        socket.on('status', (data) => {
            ...
            if (data.msg === 'Blazer Started') {
                setStatus('Model Connected');
            } else if (data.msg === 'Blazer Stopped') {
                setStatus('Connected');
            }
        });
    ...
    return (
        <div className="h-screen w-screen bg-black text-purple-100 font-mono overflow-hidden flex flex-col relative selection:bg-purple-900 selection:text-white">
            ...
            {isVideoOn && isHandTrackingEnabled && (
                <div
                    className={`fixed w-6 h-6 border-2 rounded-full pointer-events-none z-[100] transition-transform duration-75 ${isPinching ? 'bg-purple-400 border-purple-400 scale-75 shadow-[0_0_15px_rgba(34,211,238,0.8)]' : 'border-purple-400 shadow-[0_0_10px_rgba(34,211,238,0.3)]'}`}
                    style={{
                        left: cursorPos.x,
                        top: cursorPos.y,
            ...
            {/* Top Bar (Draggable) */}
            <div className="z-50 flex items-center justify-between p-2 border-b border-purple-500/20 bg-black/40 backdrop-blur-md select-none sticky top-0" style={{ WebkitAppRegion: 'drag' }}>
                <div className="flex items-center gap-4 pl-2">
                    <h1 className="text-xl font-bold tracking-[0.2em] text-purple-400 drop-shadow-[0_0_10px_rgba(34,211,238,0.5)]">
                        Blazer
                    </h1>
                    <div className="text-[10px] text-purple-700 border border-purple-900 px-1 rounded">
                        V2.0.0
                    </div>
                    {/* FPS Counter */}
                    {isVideoOn && (
                ...
                <div className="flex items-center gap-2 pr-2" style={{ WebkitAppRegion: 'no-drag' }}>
                    {/* Live Clock */}
                    <div className="flex items-center gap-1.5 text-[11px] text-purple-300/70 font-mono px-2">
                        <Clock size={12} className="text-purple-500/50" />
                        <span>{currentTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                    <button onClick={handleMinimize} className="p-1 hover:bg-purple-900/50 rounded text-purple-500 transition-colors">
                        <Minus size={18} />
                    </button>
                    <button onClick={handleMaximize} className="p-1 hover:bg-purple-900/50 rounded text-purple-500 transition-colors">
            ...
            <div className="flex-1 relative z-10 flex flex-col items-center justify-center">
                ...
                <div className="absolute top-[70px] left-1/2 -translate-x-1/2 text-purple-500 text-xs font-mono tracking-widest pointer-events-none z-50 bg-black/50 px-2 py-1 rounded backdrop-blur-sm border border-purple-500/20">
                ...
                >
                    ...
                    <div className="relative border border-purple-500/30 rounded-lg overflow-hidden shadow-[0_0_20px_rgba(6,182,212,0.1)] w-80 aspect-video bg-black/80">
                        ...
                        <div className="absolute top-2 left-2 text-[10px] text-purple-400 bg-black/60 backdrop-blur px-2 py-0.5 rounded border border-purple-500/20 z-10 font-bold tracking-wider">CAM_01</div>
                ...
                {showCadWindow && (
                    ...
                    >
                        ...
                        <div
                            data-drag-handle
                            className="h-8 bg-gray-900/80 border-b border-purple-500/20 flex items-center justify-between px-3 cursor-grab active:cursor-grabbing shrink-0"
                        >
                            <span className="text-xs font-bold tracking-widest text-purple-500/70">CAD PROTOTYPE</span>
                            <button
